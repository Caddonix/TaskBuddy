...
class API {
  ...
  Future<http.Response> postRequest(String url,
      {dynamic body, required SharedPreferences prefs}) async {
  ...
  Future<bool> saveToDo(
      ...
      required List<Map<String, dynamic>> updatedList}) async {
    ...
    if (response.statusCode == 200) {
      ...
      Fluttertoast.showToast(
          msg: "Cannot fetch ToDos! Check your internet connectivity.");
      return false;
    }
  }
  ...
  Future<List<Map<String, dynamic>>?> fetchToDos(
    ...
    // try {
      final response = await getRequest(
          "http://task-buddies.herokuapp.com/user/gettodo", prefs);
      print(response.body);
      if (response.statusCode == 200) {
  ...
  Future<List<String>> fetchCategories(
    ...
    try {
      final response = await getRequest(
          "http://task-buddies.herokuapp.com/user/getcategories", prefs);
      if (response.statusCode == 200) {
        ...
        print(response.body);
        Fluttertoast.showToast(
            msg: "Cannot fetch Categories! Check your internet connectivity.");
        return [];
      }
    } catch (e) {
      print(e);
  ...
  Future<bool> logoutUser(
    ...
    try {
      final response = await getRequest(
          'http://task-buddies.herokuapp.com/user/logout', prefs);
